...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None):
    ...
    if position_ids is None:
        # Unsqueeze to match tensor dimensions
...
class Qwen3Attention(nn.Module): 
    def __init__(self, config):
        ...
        self.head_dim = config.head_dim if config.head_dim is not None else config.n_embed // config.n_head
        self.n_kv_embed = config.n_kv_heads * self.head_dim
...
class Qwen3DenseModel(nn.Module):
    ...
    def forward(self, x, position_ids):
        B, T, C = x.size()
        cos, sin = self.rotary_emb.cos_cached[:T] , self.rotary_emb.sin_cached[:T]
...
class Qwen3MoEMLP(nn.Module) : 
    ...
    def forward(self,  x) : 
        ...
        topk_scores , topkk_indices = torch.topk(scores , self.num_experts_per_tok , dim=-1)
